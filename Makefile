...
theme: ## Install a theme.
	npm install --no-save --prefix web/themes
	npm run build --prefix web/themes
	lando drush cr
...
ciam_set: ## Set ciam keys
	lando drush cset gigya.settings gigya.gigya_api_key 3_HOC1yxUdbaD_176mJ7m396KruJQ36YZ7sYHgPViSq5uRtTRyxr1sPjuclQaOMprs -y
	lando drush cset gigya.settings gigya.gigya_application_key AAxqDHRugTSI -y
	lando drush cset gigya.settings gigya.gigya_application_secret_key LeOJs6P3V7X2R8seSPbR8zZzZDJ3YWxDcUNHM1JvSDRIMUJRZ0xUV00vR3ZhSTkrdHVKSlFwMkZieXVselZZMTdpRUQvK2I5eG1RT1prUFc= -y
	lando drush cset gigya.settings gigya.gigya_data_center us1.gigya.com -y

	lando drush cset gigya_raas.screensets gigya.login_screenset comienzosano.nestle.cl-1-RegistrationLogin-D8 -y
	lando drush cset gigya_raas.screensets gigya.login_screenset_mobile comienzosano.nestle.cl-1-RegistrationLogin-D8 -y
	lando drush cset gigya_raas.screensets gigya.profile_screenset comienzosano.nestle.cl-1-ProfileUpdate-D8 -y
	lando drush cset gigya_raas.screensets gigya.profile_screenset_mobile comienzosano.nestle.cl-1-ProfileUpdate-D8 -y			
...
phpcs: ## Check code style.
	...
	#lando exec appserver -- phpstan analyse --configuration=./grumphp_configs/phpstan/phpstan.neon --no-ansi --no-progress --error-format=table ${ARG_1}
...
test: ## Run test.
	lando exec appserver -- bin/phpunit ${ARG_1}
...
term_fox_brand: ## Terminus brand rollout: make term_fox_brand brand_id brand_title brand_url domain
	$(MAKE) term_auth
	...
	PRINT Edit and publish /taxonomy/term/@brand_id/edit;\
...
term_fox_brand_migrate: ## Terminus brand migrate: brand_id brand_title domain
	...
	IF @count <> 1 THEN SELECT tid WHERE field_domain_access = @brand AND status = 0 AND name='@brand_title' INTO brand_tid;\
	IF @count <> 1 THEN QUIT;\
	SET main_tid TO @brand_tid.0.tid;\
	REPLACE field_domain_source WITH @brand, field_brand_key WITH @brand_title;\
	...
	IF @count <> 1 SELECT tid,machine_name WHERE field_domain_access = @brand AND status = 1 INTO old_brand_tid;\
	IF @count <> 1 THEN QUIT;\
	REPLACE name WITH 'Brand @brand_title old', machine_name WITH brand_@brand_old;\
	PRINT Changed the old brand TYPE success;\
	\
	SELECT nid FROM node.tool WHERE field_domain_access = @brand AND status=0 LIMIT 1 INTO tool;\
	SELECT nid FROM node.faq WHERE status=1 ORDER BY changed DESC LIMIT 1 INTO faq;\
	\
	USE taxonomy_term.brand;\
	SELECT tid WHERE field_domain_access = @brand AND status = 0 AND tid <> @main_tid INTO childs;\
	REPLACE field_domain_source WITH @brand, field_brand_key WITH @brand_title, parent WITH @main_tid FOR @childs;\
	IF @tool.0.nid > 0 THEN REPLACE field_tools_cards WITH @tool.0.nid FOR @childs;\
	IF @faq.0.nid > 0 THEN REPLACE field_faq_cards WITH @faq.0.nid FOR @childs;\
	\
	SEEK @main_tid;\
	IF @tool.0.nid > 0 THEN REPLACE field_tools_cards WITH @tool.0.nid;\
	IF @faq.0.nid > 0 THEN REPLACE field_faq_cards WITH @faq.0.nid;\
...
term_fox_brand_migrate_test: ## Terminus brand migrate test: brand_id brand_title domain
	...
	IF @count = 0 THEN SELECT tid FROM taxonomy_term.brand WHERE field_domain_access = @brand AND status = 1 AND name='@brand_title' INTO brand_tid;\
	IF @count = 0 THEN QUIT;\
	\
